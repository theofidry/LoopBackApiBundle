...
Feature: Order filter on collections
  ...
  I need to be able to filter on properties of a related entity

  Scenario: Check fixtures
    Given the database is empty
    Given the fixtures file "search-dummy-related.yml" is loaded
...
#    And the JSON node "hydra:member[0]->relatedDummy->@id" should be equal to "/related_dummies/1"
#    And the JSON node "hydra:member[0]->relatedDummy->name" should be equal to "AAA"
#    And the JSON node "hydra:member[0]->relatedDummy->dummyDate" should be equal to "2015-04-28T02:23:50+00:00"
#    And the JSON node "hydra:member[0]->relatedDummy->enabled" should be equal to true
#    And the JSON node "hydra:member[0]->relatedDummy->price" should be equal to 10.99
#
#    And the JSON node "hydra:member[1]->@id" should be equal to "/dummies/2"
#    And the JSON node "hydra:member[1]->relatedDummy->@id" should be equal to "/related_dummies/2"
#    And the JSON node "hydra:member[1]->relatedDummy->name" should be null
#    And the JSON node "hydra:member[1]->relatedDummy->dummyDate" should be equal to "2015-04-28T02:05:50+00:00"
#    And the JSON node "hydra:member[1]->relatedDummy->enabled" should be null
#    And the JSON node "hydra:member[1]->relatedDummy->price" should be null
#
#    And the JSON node "hydra:member[2]->@id" should be equal to "/dummies/3"
#    And the JSON node "hydra:member[2]->relatedDummy->@id" should be equal to "/related_dummies/3"
#    And the JSON node "hydra:member[2]->relatedDummy->name" should be equal to ""
#    And the JSON node "hydra:member[2]->relatedDummy->dummyDate" should be null
#    And the JSON node "hydra:member[2]->relatedDummy->enabled" should be null
#    And the JSON node "hydra:member[2]->relatedDummy->price" should be equal to 0
#
#    And the JSON node "hydra:member[3]->@id" should be equal to "/dummies/4"
#    And the JSON node "hydra:member[3]->relatedDummy->@id" should be equal to "/related_dummies/4"
#    And the JSON node "hydra:member[3]->relatedDummy->name" should be null
#    And the JSON node "hydra:member[3]->relatedDummy->dummyDate" should be equal to "2016-04-28T02:23:50+00:00"
#    And the JSON node "hydra:member[3]->relatedDummy->enabled" should be equal to the boolean false
#    And the JSON node "hydra:member[3]->relatedDummy->price" should be equal to 120


  @firstLevel @noOperator @iriValue
  Scenario: First level search without operator on a string value
    When I send a "GET" request to "/dummies?filter[where][relatedDummy.anotherDummy.id]=/another_dummies/1"
    Then the JSON node "hydra:totalItems" should be equal to 1
    And the JSON node "hydra:member[0]->relatedDummy->@id" should be equal to "/related_dummies/1"

    When I send a "GET" request to "/dummies?filter[where][relatedDummy.id]=1"
    Then the JSON node "hydra:totalItems" should be equal to 1
    And the JSON node "hydra:member[0]->@id" should be equal to "/related_dummies/1"

    When I send a "GET" request to "/dummies?filter[where][relatedDummy.id]=/dummies/99"
    Then the JSON node "hydra:totalItems" should be equal to 0

    When I send a "GET" request to "/dummies?filter[where][relatedDummy.id]=invalidValue"
    Then the JSON node "hydra:totalItems" should be equal to 0
